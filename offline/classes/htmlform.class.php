<?php
class htmlform extends htmlattributes
{
	function __construct($formname = 'form1', $action = '', $method = 'POST')
	{
		...
		$this->postback = new postback($this->formdata['name']);
	...
	public function addinput($name, $type = 'text', $value = '', $group = '', $checked = FALSE)
	{
		...
		if ($type == 'textarea')
		{
			$inputdata['textarea'] = new htmltextarea($inputdata['name']);
	...
	public function top()
	{
		...
		$formdatasave->savedata($tosave, $datasavename, $formdatasave->browsersession());
